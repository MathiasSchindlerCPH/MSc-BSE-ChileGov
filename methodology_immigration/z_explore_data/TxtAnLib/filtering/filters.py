...
def f_authors(DF: pd.DataFrame, authors_list: list) -> pd.DataFrame:
    '''
    Objective: Receive as input a list of authors and returns a data frame with the tweets from these authors
...
def f_dates(DF: pd.DataFrame, start_time ="2020-10-31", end_time = "2022-04-12") -> pd.DataFrame:
    '''
    Objective: Receive an start time and end time and return the data frame only with the tweets in this period.
    Author: Andrés Couble
    Date: June 3, 2022
    '''
    df_1 = DF.copy()
...
def f_words(DF: pd.DataFrame, list_of_words: list) -> pd.DataFrame:
    '''
    Objective: Receive a list of words and return a data frame only with the tweets that contain one of these words.
    Author: Andrés Couble
    Date: June 3, 2022
    '''
    df_1 = DF.copy()
...
def f_hashtags(DF: pd.DataFrame, list_of_hashtags: list) -> pd.DataFrame:
    '''
    Objective: Receive a list of hashtags and return a data frame only with the tweets that contain one of these hashtags.
    Author: Andrés Couble
    Date: June 3, 2022
    '''
    df_1 = DF.copy()
...
def f_metrics(DF: pd.DataFrame, metric: str , threshold: int) -> pd.DataFrame:
    '''
    ...
    '''
    if metric == "Likes":
        return DF[DF['public_metrics.like_count'] >= threshold].copy()
    
    if metric == "Retweets":
        return DF[DF['public_metrics.retweet_count'] >= threshold].copy()
    
    if metric == "Quotes":
        return DF[DF['public_metrics.quote_count'] >= threshold].copy()
    
    if metric == "Replies":
        return DF[DF['public_metrics.reply_count'] >= threshold].copy()
    

def f_location(DF: pd.DataFrame, list_of_locations: list) -> pd.DataFrame:
    '''
    Objective: Receive a list of words and return a data frame only with the tweets from authors that have one of these words in the location.
    Author: Andrés Couble
    Date: June 3, 2022
    '''
    df_1=DF.copy()
...
def f_verified(DF):
    '''
    Objective: Return data frame only with tweets from verified accounts
    Date: June 22, 2022
    '''
    df_1=DF.copy()
    df_1['author.verified']=df_1['author.verified'].astype(bool)
    return df_1[df_1['author.verified']==True]

def f_minfollowers(DF,min_followers=200):
    '''
    Objective: Return data frame only with tweets from accounts with a minimum number of followers
    Date: June 22, 2022
    '''
    df_1=DF.copy()
    df_1['author.public_metrics.followers_count']=df_1['author.public_metrics.followers_count'].astype(int)
    return df_1[df_1['author.public_metrics.followers_count']>= min_followers]